-- PROJECT PILOT 4.0 CUMULATIVE UPGRADE
-- Generated August 16, 2026.
-- Safe path for the existing Project Pilot database when you are unsure whether Sprint 3.3 / 3.6 was installed.
-- This script reapplies the idempotent 015 + 016 + 017 schema upgrades in order.
...
BEGIN
  IF to_regclass('public.permit_cases') IS NULL OR to_regclass('public.profiles') IS NULL OR to_regclass('public.projects') IS NULL THEN
    RAISE EXCEPTION 'This database is missing the Project Pilot base schema. Use RUN_THIS_IN_SUPABASE_FRESH_4_0.sql for a new database instead.';
  END IF;
END $$;
...
BEGIN
  IF to_regclass('public.permit_service_orders') IS NULL THEN
    RAISE EXCEPTION 'Project Pilot 4.0 verification failed: permit_service_orders was not created.';
  END IF;
  IF to_regclass('public.permit_service_authorizations') IS NULL THEN
    RAISE EXCEPTION 'Project Pilot 4.0 verification failed: permit_service_authorizations was not created.';
  END IF;
END $$;

SELECT 'Project Pilot 4.0 cumulative upgrade complete' AS result;
