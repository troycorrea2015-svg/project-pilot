...
export default function DashboardPage() {
  ...
  const [dashboardSliderPaused, setDashboardSliderPaused] = useState(false);
  ...
  const activeDashboardCategory = PROJECT_CATEGORIES[dashboardSlideIndex];
  ...
  return (
    <main className={`dashboardPage ${projects.length ? "" : "firstRunDashboard"}`.trim()}>
      ...
      <section className="dashboardMain">
        ...
        <section className="dashboardShowcase" aria-label="Project slideshow">
          <div className="sectionTitleRow dashboardShowcaseHeading">
            <div>
              <p>PROJECT SHOWCASE</p>
              <h2>Explore common projects before you start.</h2>
            </div>
            <span>Project Pilot can guide each one step by step.</span>
          </div>
          <div
            className="dashboardProjectSlider"
            onMouseEnter={() => setDashboardSliderPaused(true)}
            onMouseLeave={() => setDashboardSliderPaused(false)}
          >
            <div className="dashboardSlideMain" key={activeDashboardCategory.key}>
              <img src={activeDashboardCategory.image} alt={`${activeDashboardCategory.title} example`} />
              <div className="dashboardSlideShade" />
              <div className="dashboardSlideCopy">
                <span>{activeDashboardCategory.label}</span>
                <h3>{activeDashboardCategory.title}</h3>
                <p>Start this project with Su and Project Pilot will organize the scope, permit route, documents, and next steps.</p>
                <button type="button" onClick={() => setProjectIdea(`I want to start a ${activeDashboardCategory.projectType.toLowerCase()} project`)}>Use this idea</button>
              </div>
              <button className="dashboardSlideArrow left" type="button" onClick={() => setDashboardSlideIndex((current) => (current - 1 + PROJECT_CATEGORIES.length) % PROJECT_CATEGORIES.length)} aria-label="Previous project">‹</button>
              <button className="dashboardSlideArrow right" type="button" onClick={() => setDashboardSlideIndex((current) => (current + 1) % PROJECT_CATEGORIES.length)} aria-label="Next project">›</button>
            </div>
            <div className="dashboardSlideThumbs">
              {PROJECT_CATEGORIES.map((item, index) => (
                <button key={item.key} className={index === dashboardSlideIndex ? "active" : ""} type="button" onClick={() => setDashboardSlideIndex(index)}>
