...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    async function loadAdmin() {
      ...
      const criticalError = [summaryResult.error, marketplaceResult.error, feedbackResult.error, contractorResult.error, creditResult.error, healthResult.error].find(Boolean);
      const secondaryErrors = [permitCaseResult.error, conciergeRequestResult.error, permitOrderResult.error, referralResult.error, permitCreditResult.error].filter(Boolean);
      if (criticalError) {
        if (criticalError.message?.includes("admin_marketplace_summary")) setError("Run migration 010 in Supabase to activate marketplace reporting.");
        else setError(criticalError.message);
      } else if (secondaryErrors.length) {
        const secondaryMessage = secondaryErrors[0]?.message || "";
        if (
          secondaryMessage.includes("permit_service_orders") ||
          secondaryMessage.includes("permit_concierge_requests") ||
          secondaryMessage.includes("referral_attributions") ||
          secondaryMessage.includes("permit_service_credit_ledger") ||
          secondaryMessage.includes("permit_cases")
        ) {
          setNotice("Some advanced admin tables are still syncing or waiting on the latest deployment. Core admin controls remain available.");
        } else {
          setNotice("Some secondary admin data could not be loaded. Refresh after the latest deployment if needed.");
        }
      }
