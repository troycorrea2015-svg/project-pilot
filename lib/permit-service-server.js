...
export async function activatePermitServiceOrder(service, session) {
  ...
  const { data: request, error: requestError } = await service
    ...
    .update({
      ...
      current_phase: "intake_review",
      concierge_summary: serviceSummaryForStatus("intake_review"),
  ...
  const { error: caseUpdateError } = await service
    ...
    .update({
      ...
      next_action: nextActionForPermitStatus("intake_review"),
  ...
  await service.from("projects").update({
    progress: Math.max(Number(currentProject?.progress || 0), projectProgressForPermitStatus("intake_review")),
    status: projectStatusForPermitStatus("intake_review"),
    next_step: nextActionForPermitStatus("intake_review"),
    updated_at: now,
  }).eq("id", order.project_id).eq("user_id", order.user_id);
