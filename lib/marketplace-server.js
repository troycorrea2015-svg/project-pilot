...
export async function createPermitServiceCheckoutSession({
  amountCents,
  basePriceCents,
  ...
  params.set("line_items[0][price_data][currency]", "usd");
  params.set("line_items[0][price_data][unit_amount]", String(amountCents));
  params.set("line_items[0][price_data][product_data][name]", "Project Pilot Permit Concierge");
  const creditText = Number(creditAppliedCents || 0) > 0
    ? ` Base price $${(Number(basePriceCents || amountCents) / 100).toFixed(2)}; $${(Number(creditAppliedCents) / 100).toFixed(2)} Project Pilot credit applied.`
    : "";
  params.set("line_items[0][price_data][product_data][description]", `Project Pilot handles the administrative permit workflow.${creditText} Government fees and licensed-professional services are separate.`);
  params.set("line_items[0][quantity]", "1");
  params.set("metadata[purchase_type]", "permit_service");
  params.set("metadata[permit_order_id]", orderId);
  params.set("metadata[permit_request_id]", requestId);
  params.set("metadata[project_id]", projectId);
  params.set("payment_intent_data[metadata][purchase_type]", "permit_service");
  params.set("payment_intent_data[metadata][permit_order_id]", orderId);
  params.set("payment_intent_data[metadata][permit_request_id]", requestId);
  params.set("payment_intent_data[metadata][project_id]", projectId);
...
export async function createStripeRefund({ paymentIntentId, amountCents, idempotencyKey, reason = "Project Pilot approved payment adjustment" }) {
