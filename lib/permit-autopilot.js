...
function uniqueDocuments(items) {
  ...
  for (const item of items) {
    ...
    if (!existing || (item.required && !existing.required)) byLabel.set(key, item);
  }
...
export function buildPermitBlueprint({ project, permitResult, user }) {
  ...
  const documentsVerified = permitResult?.documentRequirementsVerified === true || permitResult?.requirementsVerified === true;
  const baseDocuments = [
    { key: "ownership", label: "Proof of ownership or owner authorization", required: false, recommended: true, source: "Project Pilot", verification: "Only required if the authority asks for it" },
    { key: "application_id", label: "Applicant identification and contact information", required: false, recommended: true, source: "Project Pilot", verification: "Project Pilot collects the information first; upload ID only if the authority requires it" },
  ];
  const projectDocuments = (PROJECT_DOCUMENTS[projectType] || PROJECT_DOCUMENTS.general).map(([key, label]) => ({
    key,
    label,
    required: false,
    recommended: true,
    source: "Project type",
    verification: "Project Pilot will confirm whether your jurisdiction actually requires this before submission",
  }));
  const authorityDocuments = (permitResult?.documents || []).slice(0, 8).map((label, index) => ({
    key: `authority_${index + 1}`,
    label: String(label),
    required: documentsVerified,
    recommended: true,
    source: permitResult?.jurisdiction || "Matched authority",
    verification: documentsVerified ? "Verified authority requirement" : "Likely requirement — confirmation pending",
  }));
  const incorporatedPlace = permitResult?.locationGeography?.incorporatedPlace;
  const municipalDocuments = incorporatedPlace
    ? [{ key: "municipal_approval", label: `${incorporatedPlace} zoning or municipal approval, when required`, required: permitResult?.municipalApprovalVerifiedRequired === true, recommended: true, source: "Jurisdiction route", verification: "Project Pilot will confirm whether this applies to your address" }]
    : [];
  ...
  return {
    projectType,
    questions,
    checklist: uniqueDocuments([...authorityDocuments, ...municipalDocuments, ...projectDocuments, ...baseDocuments]),
    documentRequirementsVerified: documentsVerified,
