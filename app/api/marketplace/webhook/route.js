...
import { activatePermitServiceOrder, cancelPendingPermitServiceOrder } from "../../../../lib/permit-service-server";

export const runtime = "nodejs";

async function completeMarketplaceLead(service, session) {
  ...
  if (Number(session.amount_total || 0) !== Number(currentMatch.fee_cents || 0)) throw new Error("Stripe payment amount does not match the introduction fee.");

  if (currentMatch.status === "Accepted" && currentMatch.payment_status === "Paid") return;
  if (currentMatch.status !== "Offered" || currentMatch.payment_status !== "Pending") throw new Error("The opportunity is not in a payable state.");
  ...
  await service.from("marketplace_lead_requests").update({ status: "Matched", updated_at: new Date().toISOString() }).eq("id", match.lead_request_id).eq("status", "Open");
...
export async function POST(request) {
  ...
  try {
    ...
    const { data: existing } = await service.from("stripe_webhook_events").select("event_id").eq("event_id", event.id).maybeSingle();
    if (existing) return NextResponse.json({ received: true, duplicate: true });

    if (event.type === "checkout.session.completed") {
      const session = event.data?.object || {};
      if (session.metadata?.purchase_type === "permit_service" || session.metadata?.permit_order_id) {
        await activatePermitServiceOrder(service, session);
      } else {
        await completeMarketplaceLead(service, session);
      }
    }
    ...
    if (event.type === "checkout.session.expired") {
      const session = event.data?.object || {};
      if (session.metadata?.purchase_type === "permit_service" || session.metadata?.permit_order_id) {
        await cancelPendingPermitServiceOrder(service, session);
      } else {
        const matchId = session.metadata?.lead_match_id || session.client_reference_id;
        if (matchId) {
          await service
    ...
    const { error: eventInsertError } = await service.from("stripe_webhook_events").insert({ event_id: event.id, event_type: event.type });
