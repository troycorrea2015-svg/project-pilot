    ...
    <main className="homePage">
      ...
      <section className="heroPanel" id="top">
        <div className="heroCopy">
          ...
          <p className="heroLead">AI-powered guidance for permits, planning, budgets, and the right pros — all in one homeowner workspace.</p>
          ...
          <div className="heroTrustRow">
            <div><b>◇</b><span><strong>Permit Confidence</strong><small>Guided permit steps with clear explanations</small></span></div>
            <div><b>♙</b><span><strong>Trusted Pros</strong><small>Connect with vetted contractors</small></span></div>
        ...
        <div className="previewMain">
          ...
          <div className="previewColumns">
            <section className="projectPanel">
              <div className="panelTitle"><h3>My Projects</h3><a href="#access">View all →</a></div>
              {projects.map(([title, statusLabel, progress, next], index) => (
                <article className="projectRow" key={title}>
                  ...
                  <div className="projectDetails"><strong>{title}</strong><small>Austin, TX</small><div><mark>{statusLabel}</mark><span className="bar"><i style={{ width: `${progress}%` }} /></span><em>{progress}%</em></div></div>
                  <div className="projectNext"><small>Next up</small><strong>{next}</strong></div>
                </article>
              ))}
            </section>
            <aside className="contractorPanel">
              <div className="panelTitle"><h3>Find Contractors</h3><a href="/contractors">View all →</a></div>
              {contractors.map((name, index) => (
                <article key={name}><div className={`contractorPhoto contractor${index + 1}`} /><span><strong>{name}</strong><small>Austin, TX · ★ 4.9</small></span><button type="button">View Profile</button></article>
              ))}
      ...
      <section className="launchSection" id="resources">
        ...
        <div className="launchGrid">
          <div className="launchChecks">{launchChecks.map(item => <div key={item}><span>✓</span><p>{item}</p></div>)}</div>
