-- PROJECT PILOT 4.5 CUMULATIVE UPGRADE (schema unchanged from 4.4)
...
BEGIN
  IF to_regclass('public.permit_cases') IS NULL OR to_regclass('public.profiles') IS NULL OR to_regclass('public.projects') IS NULL THEN
    RAISE EXCEPTION 'This database is missing the Project Pilot base schema. Use RUN_THIS_IN_SUPABASE_FRESH_4_5.sql for a new database instead.';
  END IF;
END $$;
...
BEGIN
  IF to_regclass('public.permit_service_orders') IS NULL THEN
    RAISE EXCEPTION 'Project Pilot 4.2 verification failed: permit_service_orders was not created.';
  END IF;
  IF to_regclass('public.permit_service_authorizations') IS NULL THEN
    RAISE EXCEPTION 'Project Pilot 4.2 verification failed: permit_service_authorizations was not created.';
  END IF;
  IF to_regclass('public.referral_codes') IS NULL THEN
    RAISE EXCEPTION 'Project Pilot 4.2 verification failed: referral_codes was not created.';
  END IF;
  IF to_regclass('public.permit_service_credit_ledger') IS NULL THEN
    RAISE EXCEPTION 'Project Pilot 4.2 verification failed: permit_service_credit_ledger was not created.';
  END IF;
END $$;

SELECT 'Project Pilot 4.2 cumulative upgrade complete' AS result;
